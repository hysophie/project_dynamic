...
- 승우가 옷을 사기위해 **노트북**을 통해 **서울마트**에 접속했다. (client_id = '가나다' 발급, session_id='ABC' 발급, session_seq = 1)
- 서울마트에서 옷을 사지 않고 **우리홈쇼핑**에 접속해서 **옷을 샀다**. (client_id = '가나다' 유지, session_id='DEF' 발급, session_seq = 2) -> 이 경우의 데이터만 남음
- 다음날 **핸드폰**으로 모자를 사기위해 **스마트쇼핑몰**에 접속했다. (client_id = '다라마' 발급, session_id='XYZ', session_seq = 1)
