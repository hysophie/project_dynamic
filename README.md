...
2. feature modeling
- Session, Pruduct 데이터를 중심으로 전처리와 변수 추가.(code uploaded), Search1,2는 idea제시  
- Session
...
- Search1,Search2
  ...
  - 두 데이터셋의 활용 법은 분석방식을 어떻게 할지에 따라 달라질 것으로 예상. 
    - 전체검색량 대비 개인검색량을 개인적 관심의 측도로 사용.
    - 검색량 추세선의 기울기를 하나의 변수로 사용. (회귀직선의 기울기와 동일한 의미)
    - 그 외에도 search1, search2는 변수들 간의 조합을 통해 유의미한 변수를 찾을 수 있을 것으로 예상 -> 검색어 데이터 활용법을 조사해도 좋을 듯? 
