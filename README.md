...
3. 구매확률이 높은 session 중에 가격이 높아져도 구매 가능성이 높은 session 패턴 분석/ 
   구매확률이 높은 session 중에 가격이 낮아졌을 때 소비가 높아질 높은 패턴 분석
 - 이 인덱스에 대해서 좀 더 논의가 필요할 듯
 
-> 비록 가격을 바꾼 후 A/B test를 통해서 실제 효과를 볼 수 없는게 아쉽.. 
