 ...
 * NA value 채우기
   ...
   - CLNT_AGE: NaN 값의 분포가 'CLNT_GENDER'와 정확히 일치 > 비회원 구매일 것.
    - 다른 변수와의 관계를 통해 채워보려고 했으나, 상관관계의 절대값이 0.1을 넘는 변수도 SESS_SEQ 하나 뿐(0.15) (하지만 SESS_SEQ가 성별과의 연관성을 말해줄 수는 없다.)  
    - softmax나 RF_Classifier 같은 모델을 적용해보려고도 했지만, 1. 해당 모델들은 NaN 값이 있는 변수들에서 작동하지 않는다. 중요변수인 CLNT_AGE, CLNT_GENDER를 predict하기 위해 자의적으로 다른 변수들의 결측값을 채우는 것은 옳지 않다. -> 채우더라도 BIAS가 상당할 것 2. 여타 변수들이 CLNT_AGE와 큰 연관을 맺고 있는 것 같지도 않다. - CLNT_AGE, CLNT_GENDER 결측치의 관찰값 비율은 약 12% 정도.
    -> 아예 해당 row를 지워버리기로 결정. :약 156만 건 남음.
