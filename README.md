...
동건:
- Search1,2 데이터 전처리
  -  개인이 검색한 검색어의 종류 -> KWD_CNT
  - *검색어의 종류와 상관없이* 개인의 총 검색량 aggregation -> SEARCH_CNT
  - *검색어의 종류와 상관없이* 검색어의 당일 전체 검색량 -> SEARCH_TOT 
  - *검색어의 종류와 상관없이* 개인의 총 검색량 / 검색어의 당일 전체 검색량 -> SEARCH_RATIO
  - EX) SEARH_CNT = 5, KWD_CNT = 2, SEARCH_RATIO = 0.01 이면, 고객은 한세션에서 2개의 검색어를 가지고 5번 검색을 했고, 두 개의 검색어는 당일 총 500번 검색되었다. 따라서 SERCH_RATIO = 5/500 이 됨을 알 수 있다.  
