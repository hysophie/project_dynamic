...
승우:
  - Act1
    : Session 데이터 내 전처리 (자료형에 집중)
    : datetime 모듈을 이용하여, 2018년 4월~9월 사이의 모든 주말 정보를 얻어오고,
      2018년 국가 지정 공휴일 자료를 검색을 통해 list로 만들어, 이들을 통합해 4~9월 사이의 모든 휴일 리스트를 만듦.
      > Session 데이터에 Rest 변수를 새로 만들어, 휴일 구입시 1, 그렇지 않을시 0을 labeling.
