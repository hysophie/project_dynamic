 ...
 * NA value 채우기
   ...
   - CLNT_AGE: NaN 값의 분포가 'CLNT_GENDER'와 정확히 일치 > 비회원 구매일 것.
    ...
    - softmax나 RF_Classifier 같은 모델을 적용해보려고도 했지만, 여타 변수들이 CLNT_AGE와 큰 연관을 맺고 있는 것 같지 않을 뿐더러, 모든 row에 대  해 이 두 변수가 결측되어 있는데, 이를 임의로 predict한다는 것은 BIAS를 높이는 길이라 생각. 해당 결측치의 관찰값 비율은 약 12% 정도.
    -> 아예 해당 row를 지워버리기로 결정.
